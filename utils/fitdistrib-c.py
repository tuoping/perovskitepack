...
for T in [i for i in range(100, 145, 10)]+[i for i in range(155, 440, 20)]:
    ...
    if T < 155:
        ...
        dirname = "T"+str(T)
    print(dirname)
    _data = np.loadtxt(dirname+"/c_distances")
    data = pd.DataFrame(_data, columns=["number"])
    data["cut_group"] = pd.cut(data["number"], 200)
    ...
    # else:
    popt, pcov = curve_fit(function_1G, x, y, bounds=([6, 0.1, 1000], [7, 0.5, 5000]))
    ffit.write("%d    %f  %f\n"%(T, popt[0], popt[0]))
    y_pred = [function_1G(i,*popt) for i in x]
    print(popt)
    plt.scatter(x,y_pred,marker="s")
    
    plt.savefig(dirname+"/distrib-cdistances.png")
    plt.show()
