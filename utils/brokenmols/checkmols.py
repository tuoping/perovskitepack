...
class Molecule(object):
    ...
    def check_distance(self, mol):
        ...
        atype_mol = ["C", "N", "N", "H", "H", "H", "H", "H"]
        dist_mat = get_distances(self.coords_mol, mol.coords_mol, self.cell, pbc=True)[1]
        for i in range(len(self.indices_mol)):
            for j in range(len(mol.indices_mol)):
                connection_type = atype_mol[i]+"-"+atype_mol[j]
                # print(i,j,connection_type, dist_mat[i][j],dist_mat[i][j] < cutoffs[connection_type])
                if dist_mat[i][j] < cutoffs[connection_type]:
                    return False
        return True

    def checkmol(self, num_atoms, cutoffs, species):
        if len(self.coords_mol) != num_atoms:
            return False
        dist_mat = get_distances(self.coords_mol, self.coords_mol, self.cell, pbc=True)[1]
        for i in range(num_atoms):
            for j in range(i+1, num_atoms):
                if species[i]+"-"+species[j] in cutoffs.keys() and dist_mat[i][j] > cutoffs[species[i]+"-"+species[j]]:
                    return False
        return True
...
class FAPbI3(object):
        ...
        if moltype == "FA":
            for idx_C,C in enumerate(coords_C):
                ...
                if len(indices) != 8: 
                    ...
                    dNN = get_distances(coords[1], coords[2], cell, pbc=True)[1][0][0]
                    if dNN < 2:
                        print(np.array(indices)+1)
    ...
    def setcutoff_CN_H(self, cutoff_CN = 2.0, cutoff_CH=1.5, cutoff_NH=1.5):
        self.cutoff_CN = cutoff_CN
        self.cutoff_CH = cutoff_CH
        self.cutoff_NH = cutoff_NH
...
if __name__ == "__main__":
    ...
    indices_mol = np.loadtxt("mols.dat", dtype=int)
    ...
    # checkmols
    fp_badmols = open("log.brokenmols", "w")
    for idx_mol in range(len(cubic.molecules)):
        mol = cubic.molecules[idx_mol]
        good = mol.checkmol(8, cutoffs, ["C", "N1", "N2", "H1", "H2", "H2", "H3", "H3"])
        if not good:
            fp_badmols.write("%d %d %d %d %d %d %d %d\n"%(mol.indices_mol[0]+1, mol.indices_mol[1]+1, mol.indices_mol[2]+1, mol.indices_mol[3]+1, mol.indices_mol[4]+1, mol.indices_mol[5]+1, mol.indices_mol[6]+1, mol.indices_mol[7]+1))
        else:
            for idx_mol2 in range(idx_mol+1, len(cubic.molecules)):
                mol2 = cubic.molecules[idx_mol2]
                good = mol.check_distance(mol2)
                
                if not good:
                    fp_badmols.write("%d %d %d %d %d %d %d %d\n"%(mol.indices_mol[0]+1, mol.indices_mol[1]+1, mol.indices_mol[2]+1, mol.indices_mol[3]+1, mol.indices_mol[4]+1, mol.indices_mol[5]+1, mol.indices_mol[6]+1, mol.indices_mol[7]+1))
                    fp_badmols.write("%d %d %d %d %d %d %d %d\n"%(mol2.indices_mol[0]+1, mol2.indices_mol[1]+1, mol2.indices_mol[2]+1, mol2.indices_mol[3]+1, mol2.indices_mol[4]+1, mol2.indices_mol[5]+1, mol2.indices_mol[6]+1, mol2.indices_mol[7]+1))
