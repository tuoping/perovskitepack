...
if __name__ == "__main__":
    ...
    sample_mol = dpdata.System("water.vasp", fmt="vasp/poscar")
    traj = dpdata.System(filename, fmt=fmt)
    for i_frame in range(0,traj.get_nframes()):
        stime = time.time()
        frm = traj[i_frame]
        cubic = FAPbI3(frm, fmt=fmt, type_map=["I", "Pb", 'C', 'N', 'H'])
        ...
        indices_mol = np.array(indices_mol)
        assert len(indices_mol.shape) == 2
        #np.save("indices_mol.npy", indices_mol)
        indices_tosubstitute = np.random.choice(len(indices_mol), size=1)
        for i_mol in indices_tosubstitute:
            print(i_mol)
            while(True):
                ...
                new_cubic = cubic.substitute_mol_by_molecule(i_mol, sample_mol)
